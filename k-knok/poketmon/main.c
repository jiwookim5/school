...
int poketmon_load(t_data **list)
{
    ...
    *list = (t_data *)malloc(sizeof(t_data) * cnt);
    while(i < cnt)
    {
        fscanf(fd, "%s %s %d %d", (*list)[i].name, (*list)[i].type, \
            &(*list)[i].min_attack, &(*list)[i].min_hp);
        i++;
    }
    fclose(fd);
    return(cnt);
...
void print_battle_poketmon(t_player_info *player, t_my *wild, int wild_max_hp)
{
    printf("\t\t\t\t          야생 포켓몬: %s \n\t\t\t\t          (HP: %d/%d)\n", \
                wild->name, wild->hp, wild_max_hp);
...
void fight_poketmon(t_player_info *player, t_my *wild, int wild_max_hp)
{
    int turn = rand() % 2;
    int i = 0;
    ...
    {
        wild->hp = 0;
        print_battle_poketmon(player, wild, wild_max_hp);
    }    
}
...
void attack_menu(t_player_info *player, t_my *wild, int slect, int wild_max_hp)
{
    ...
    {
        int turn = rand() % 2;
        fight_poketmon(player, wild, wild_max_hp);
    }
...
int slect_three(t_player_info *player, t_my *wild, int wild_max_hp)
{
    int escape_percent;
...
int item_monster_ball(t_player_info *player, t_my *wild, int wild_max_hp)
{
    ...
    {
        char nickname_choice;
        printf("앗! %s을(를) 잡았다!\n", wild->name);
        printf("별명을 지어주시겠습니까? (y/n): ");

        while (getchar() != '\n');
        scanf(" %c", &nickname_choice);
        if (nickname_choice == 'y' || nickname_choice == 'Y')
        {
            printf("새로운 별명 입력: ");
            scanf("%s", wild->name);
        }
...
void wild_hp_is_zero(t_player_info *player)
{
...
void set_wild(t_my *wild, t_data *list, int i, int *wild_max_hp)
{
    int wait = (rand() % 5) + 1;
    sleep(wait);
...
void item_heal(t_player_info *player)
{
    if (player->healer > 0)
    {
        player->healer--;
...
int play_adventure(t_player_info *player, t_data *list, int cnt)
{
    ...
    printf("========================================\n");
    printf("포켓몬을 탐색하는중 . . .\n");
    set_wild(&wild, list, i, &wild_max_hp);
    printf("========================================\n");
    ...
    while (j < player->poket_cnt)
    {
        ...
        {
            player->poketmon = player->my_list[j];
            found_alive = 1;
            break;
        } 
        j++;
    ...
    while(player->poketmon.hp >= 0 && wild.hp >= 0)
    {
        ...
        else if (slect == 2)
        {
            ...
            if (item_choice == 1)
            {
                int res = item_monster_ball(player, &wild, wild_max_hp);
                if (res == 1)
                    return (1);
                if (res == 2)
                    break;
        ...
        {
            if (slect_three(player, &wild, wild_max_hp) == 1)
                break;
        }
    }
    return (0);
...
void heal_poketmon(t_player_info *player)
{
    int i = 0;
...
int game_menu(t_player_info *player, t_data *list, int cnt)
{
    int action;
    scanf("%d", &action);
...
int main()
{
    ...
    while (1)
    {
        ...
        while(1)
        {
            ...
            printf(">> ");
            if (game_menu(&player, list, cnt) == 1)
                break;
        }
    }
    free(list);
